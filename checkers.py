...
class Piece:
    ...
    def __init__(self, radius: float, color: pygame.Color):
        ...
        self.rect = (
            self.__create_piece_rect()
        )   # draws onto surface and provides rect
    ...
    def __create_piece_rect(self) -> pygame.Rect:
        ...
        rect = pygame.draw.circle(
            self.surface, self.color, (self.radius, self.radius), self.radius
        )
        return rect
    ...
    def __set_pos(self) -> tuple[tuple[float]]:
        ...
        abs_pos = (
            self.col * self.size + BOARDPOSX,
            self.row * self.size + BOARDPOSY,
        )
        return (rel_pos, abs_pos)

    def change_color(self, color: pygame.Color):
        self.rect = pygame.draw.circle(
            self.surface, color, (self.radius, self.radius), self.radius
        )
...
class Board:
    ...
    def move_piece(self, piece: Piece, new_row, new_col):
        ...
        piece.abs_pos = (
            new_col * piece.size + BOARDPOSX,
            new_row * piece.size + BOARDPOSY,
        )
    ...
    def draw_valid_moves(self, valid_moves: list[Square]):
        ...
        for square in valid_moves:
            square.rect = pygame.draw.rect(
                self.surface,
                GREEN,
                (square.rel_pos, (square.size, square.size)),
            )
    ...
    def generate_up_moves(self, piece, possible_moves):
        ...
        if row - 1 >= 0 and col < SQUARECOUNT - 1:
            if self.struct[row - 1][col + 1].contents == EMPTY:
                ...
                if (
                    self.struct[row - 1][col + 1].contents.team_color
                    != piece.team_color
                ):
                    # enemy detected
                    if (
                        row - 2 >= 0
                        and col < SQUARECOUNT - 2
                        and self.struct[row - 2][col + 2].contents == EMPTY
                    ):
    ...
    def generate_valid_moves_double_steps(self, piece: Piece):
        ...
        for move in possible_moves:
            ...
            if (
                abs(move.row - piece.row) == 1
                and abs(move.col - piece.col) == 1
            ):
    ...
    def delete_piece(self, piece: Piece, pieces: Pieces):
        ...
        # first draw
        self.struct[piece.row][piece.col].surface.fill(
            self.struct[piece.row][piece.col].color
        )
        self.surface.blit(
            self.struct[piece.row][piece.col].surface,
            self.struct[piece.row][piece.col].rel_pos,
        )
        piece.active = False
    ...
    def select_move(self, piece, mouse_pos, valid_moves):
        selected_square = self.mouse_to_square(
            mouse_pos
        )   # checks if an actual square was selected
    ...
    def make_move(
        self, piece: Piece, square: Square, game_pieces: list[Pieces]
    ):
        piece_taken = False
        old_row = piece.row
        old_col = piece.col
        ...
        self.struct[old_row][old_col].contents = EMPTY
        # first draw
        self.struct[old_row][old_col].surface.fill(
            self.struct[old_row][old_col].color
        )
        self.surface.blit(
            self.struct[old_row][old_col].surface,
            self.struct[old_row][old_col].rel_pos,
        )
        piece.row = new_row
        piece.col = new_col
        ...
        if (abs(new_row - old_row)) + (abs(new_col - old_col)) == 4:
            ...
            self.delete_piece(
                self.struct[mid_row][mid_col].contents, delete_pieces
            )
            piece_taken = True
        return piece_taken
...
class State:
    ...
    def initiate_change(self):
        ...
        if self._current_state == 0:
            ...
            if not self.lock_piece:
                self.selected_piece = self.board.select_piece(
                    self.players[self.turn], mouse_pos
                )
            if self.selected_piece == None:
                ...
                if self.lock_piece:
                    self.valid_moves = (
                        self.board.generate_valid_moves_double_steps(
                            self.selected_piece
                        )
                    )
                else:
                    self.valid_moves = self.board.generate_valid_moves_steps(
                        self.selected_piece
                    )
        ...
        if self._current_state == 1:
            ...
            self.selected_move = self.board.select_move(
                self.selected_piece, mouse_pos, self.valid_moves
            )
            if self.selected_move == None:
                if self.lock_piece:
                    if (
                        self.board.mouse_to_square(mouse_pos).contents
                        == self.selected_piece
                    ):
                        return 4
                    else:
    ...
    def state_2(self):
        ...
        self.piece_taken = self.board.make_move(
            self.selected_piece, self.selected_move, self.players
        )
...
def main():
    ...
    game_state.get_components(
        checkerboard, players, turn, selected_move, selected_piece, valid_moves
    )
